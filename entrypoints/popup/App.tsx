...
function App() {
  ...
  const [fftResult, setFftResult] = useState<number | null>(null);
  ...
  useEffect(() => {
    ...
    port.onMessage.addListener((message: any) => {
      if (message.type === 'STATUS_UPDATE') {
        ...
        if (message.fftScore !== undefined) {
          setFftResult(message.fftScore);
        } else {
          setFftResult(null);
        }
        setIsClassifying(false);
  ...
  // Kombinierter Score aus Modell-Inferenz (80% Gewicht) und FFT-Frequenzanalyse (20% Gewicht)
  const combinedAiScore = fftResult !== null
    ? (artificialScore * 0.8 + (fftResult / 100) * 0.2)
    : artificialScore;
                      ...
                      <div className="results-panel">
                        {isAiDetector && (
                          <div className={`verdict-badge ${isAIImage ? 'verdict-ai' : 'verdict-real'}`}>
                            {isAIImage 
                              ? `🤖 KI-GENERIERTES BILD (${(combinedAiScore * 100).toFixed(1)}% Kombi-Score)`
                              : `📸 ECHTES BILD (${((1 - combinedAiScore) * 100).toFixed(1)}% Kombi-Score)`
                            }
                          </div>
                        )}
                        ...
                        {fftResult !== null && (
                          <div style={{ marginTop: '16px', borderTop: '1px solid var(--border-color)', paddingTop: '12px' }}>
                            <div className="result-row">
                              <div className="result-label-bar">
                                <span className="result-label">🔍 Frequenz-Rasteranalyse (FFT)</span>
                                <span className="result-percent">{fftResult}%</span>
                              </div>
                              <div className="result-progress-track">
                                <div className="result-progress-fill" style={{ 
                                  width: `${fftResult}%`,
                                  background: fftResult > 40 ? 'linear-gradient(90deg, #f59e0b, #ef4444)' : 'linear-gradient(90deg, #10b981, #3b82f6)' 
                                }} />
                              </div>
                              <p style={{ fontSize: '11px', color: 'var(--text-color)', opacity: 0.7, margin: '4px 0 0 0', lineHeight: '1.4' }}>
                                Erkennt künstliche Pixelraster im Fourier-Frequenzspektrum (typisch für ältere GANs und viele Diffusionsmodelle).
                              </p>
                            </div>
                          </div>
                        )}
